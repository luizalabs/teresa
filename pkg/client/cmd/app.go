...
func appAutoScaleSet(cmd *cobra.Command, args []string) {
	...
	if max != -1 {
		if max < 1 {
			client.PrintErrorAndExit("--max=MAXPODS must be at least 1, max: %d", max)
		}

		if min != -1 && min > max {
			client.PrintErrorAndExit("--max=MAXPODS must be larger or equal to --min=MINPODS, max: %d, min: %d", max, min)
		}
	} else if min == -1 {
